...
@attrs.define(slots=False)
class EnvLatentConditionConfig:
    """
    Remap the key from the input dictionary to the output dictionary. For `env latent condition`.
    """

    obj: LazyDict = L(ReMapkey)(output_key="env_latent_condition", dtype=None)
    dropout_rate: float = 0.0
    input_key: str = "env_latent_condition"
...
VideoUnirelightConditionerConfig: LazyDict = L(VideoUnirelightConditioner)(
    text=TextConfig(),
    fps=FPSConfig(),
    num_frames=NumFramesConfig(),
    image_size=ImageSizeConfig(),
