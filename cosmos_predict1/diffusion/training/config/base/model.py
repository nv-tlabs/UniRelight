...
@attrs.define(slots=False)
class MultiviewModelConfigRelight(DefaultModelConfig):
    n_views: int = 4
    pixel_corruptor: LazyDict = None
    condition_keys: List[str] = ["rgb"]
    condition_drop_rate: float = 0.1
    append_condition_mask: bool = True
    guidance_weights: float = None
    source_drop_rate: float = 0.0

@attrs.define(slots=False)
class MultiviewModelConfigRelightOnly(DefaultModelConfig):
    n_views: int = 4
    pixel_corruptor: LazyDict = None
    condition_keys: List[str] = ["rgb"]
    condition_drop_rate: float = 0.1
    append_condition_mask: bool = True
    basecolor_drop_rate: float = 0.5

@attrs.define(slots=False)
class MultiviewModelConfigRelightSourceConcat(DefaultModelConfig):
    n_views: int = 4
    pixel_corruptor: LazyDict = None
    condition_keys: List[str] = ["rgb"]
    condition_drop_rate: float = 0.1
...
@attrs.define(slots=False)
class MultiviewModelConfigAlbedo(DefaultModelConfig):
    n_views: int = 4
    pixel_corruptor: LazyDict = None
