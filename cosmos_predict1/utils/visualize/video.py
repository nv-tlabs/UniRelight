...
def save_img_or_video(sample_C_T_H_W_in01: Tensor, save_fp_wo_ext: Union[str, IO[Any]], fps: int = 24, save_images: bool = False) -> None:
    """
    ...
    if sample_C_T_H_W_in01.shape[1] == 1:
        ...
        # )
        ext = '.png'
        save_path = f"{save_fp_wo_ext}{ext}"
        save_obj.save(save_path)
    else:
        save_obj = rearrange((sample_C_T_H_W_in01.cpu().float().numpy() * 255), "c t h w -> t h w c").astype(np.uint8)

        if save_images:
            os.makedirs(save_fp_wo_ext, exist_ok=True)
            for ind in range(save_obj.shape[0]):
                save_path = os.path.join(save_fp_wo_ext, f"{ind:04d}.png")
                save_image = PILImage.fromarray(save_obj[ind], mode="RGB")
